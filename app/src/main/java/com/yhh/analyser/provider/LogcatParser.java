...
public class LogcatParser extends LogParser {
    private static String TAG = LogUtils.DEBUG_TAG + "LogcatParser";
    private boolean DEBUG = true;

    private BufferedWriter bw;
    private String mDir;
    public static String newFile = "statistics";
...
//            LOG_DIR = "/data/local/log/curlog";
    ...
    public void parse(Handler handler) {
        ArrayList<File> files = listTargetLog(mDir, LOG_LOGCAT);
        if (files == null || files.size() <= 0) {
            Log.w(TAG,"LOG_LOGCAT is null");
            return;
        }
