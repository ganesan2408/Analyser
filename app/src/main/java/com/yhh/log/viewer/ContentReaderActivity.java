...
import com.yhh.chart.other.SleepDataLogParser;
import com.yhh.log.analyser.MainLogAnalyser;
import com.yhh.log.model.LogModel;
import com.yhh.utils.ConstUtils;
import com.yhh.utils.DialogUtils;

public class ContentReaderActivity extends Activity implements TabListener{
    ...
    private int mMode;
    private LogModel mALog;
    private LogModel mSLog;
...
//                    SleepDataLogParser sd = new SleepDataLogParser(MainLogAnalyser.sCurrentLogDir);
//                    sd.parseTarget(new File(logPath));
//                    String sta = sd.statistic();
//                    DialogUtils.showAlergDialog(ContentReaderActivity.this, "休眠与唤醒统计", sta);
//                }else if(logPath.contains(ConstUtils.LOG_INTERRUPTES)){
//                    Log.i(TAG,"BEGIN LOG_INTERRUPTES PARSER");
//                    InterruptLogParser ip = new InterruptLogParser(logPath);
...
//            break;
    ...
    class MyHandler extends Handler {
        ...
        public void handleMessage(android.os.Message msg) {
            if (msg.what == 0x11) {        // 显示首页的log
                ...
                DialogUtils.closeLoading();
                Toast.makeText(ContentReaderActivity.this, "权限不够", Toast.LENGTH_SHORT).show();
            }
        };
    }
    ...
    private void beginSearch(final String key){
        ...
        Log.d(TAG, "Loading...");
        DialogUtils.showLoading(ContentReaderActivity.this, "正在搜索中");
    }
    ...
    private void initLogUI() {
        ...
        Log.d(TAG, "Loading...");
        DialogUtils.showLoading(ContentReaderActivity.this);
    }
    ...
    private void readFileByPage(String filePath) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                lineNum++;
                if (lineNum % LogModel.LINES_PRE_PAGE == 0) {
...
//                    if(pattern.matcher(tmpLog.get(j)).find()){
                    ...
                    arrSub++;
                    if (arrSub >LogModel.LINES_PRE_PAGE-1) {
...
//        setSearchUIVisible(false);
        if(lineNum>0){
            int pageNum = (lineNum-1)/LogModel.LINES_PRE_PAGE;
    ...
    private void showALog(int specLineNum){
        ...
        int lineNum = specLineNum % LogModel.LINES_PRE_PAGE;
                        ...
                        new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                try {
                                    ...
                                    Toast.makeText(ContentReaderActivity.this, "请输入数字",
